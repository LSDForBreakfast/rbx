...
Lavender.Ascii = "qwertyuiopasdfghjklzxcvbnmQWRTYUIOPASDFGHJKLZXCVBNM";
Lavender.GLOBAL_ENVIRONMENT = {};
Lavender.Windows = {};
Lavender.FontSemiBold = Font.new('rbxasset://fonts/families/GothamSSm.json',Enum.FontWeight.SemiBold,Enum.FontStyle.Normal);
Lavender.Flags = {};
Lavender.Colors = {
	Black = Color3.fromRGB(16, 16, 16),
	Main = Color3.fromRGB(255, 106, 133)
};
Lavender.DragBlacklist = {};
Lavender.Version = '1.6';
Lavender.Lucide = {
...
Lavender.WindowFlags = {};

function Lavender:IsMobile() : boolean
	return UserInputService.TouchEnabled;	
end;

function Lavender:RandomString() : string
...
function Lavender:GetTextSize(Text : TextLabel,CustomFont: Enum.Font) : Vector2
...
function Lavender:IsMouseOverFrame(Frame : Frame) : boolean
...
function Lavender:GetCalculatePosition(planePos: number, planeNormal: Vector3, rayOrigin: number, rayDirection: Vector3) : number
...
function Lavender:CreateHover(Element : Frame,Callback : (boolean) -> any)
...
function Lavender:GetIcon(name : string) : string
	return Lavender.Lucide['lucide-'..tostring(name)] or Lavender.Lucide[name] or Lavender.Lucide[tostring(name)] or "";
end;

function Lavender:SetIcon(name : string, value : string)
	Lavender.Lucide[name] = value;
end;

function Lavender:Rounding(num: number, numDecimalPlaces: number) : number
	local mult: number = 10 ^ (numDecimalPlaces or 0);
	return math.floor(num * mult + 0.5) / mult;
end;

function Lavender:CreateAnimation(Instance: Instance , Time: number , Style : Enum.EasingStyle , Property : {[string] : any}) : Tween
...
function Lavender:NewInput(Frame : Frame , Callback : () -> ()) : TextButton
...
function Lavender:Drag(InputFrame: Frame, MoveFrame: Frame, Speed : number)
	...
	local function updateInput(input)
		...
		Lavender:CreateAnimation(MoveFrame,Speed,nil,{
			Position = position
		});
	end;

	InputFrame.InputBegan:Connect(function(input)
		if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and #Lavender.DragBlacklist <= 0 then 
			dragToggle = true
	...
	UserInputService.InputChanged:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch and #Lavender.DragBlacklist <= 0 then
			if dragToggle then
				updateInput(input)
			end
		else
			if #Lavender.DragBlacklist > 0 then
				dragToggle = false
			end
		end
	end);
...
function Lavender:ScrollSignal(Scroll: ScrollingFrame,UIListLayout: UIListLayout,Type:string)
	if Type == 'X' then
...
function Lavender:CreateResponse(args: {[string] : (any) -> any})
	local main = {};
...
function Lavender:GetWindowFromElement(Element: GuiObject)
	for i,v in next , Lavender.Windows do
...
function Lavender:CreateOption(OptionButton: ImageButton): Elements
	Lavender:CreateHover(OptionButton,function(bool)
		if bool then
			Lavender:CreateAnimation(OptionButton,0.5,{
	...
	local OwnWindow = Lavender:GetWindowFromElement(OptionButton);
	...
	Lavender:ScrollSignal(ScrollingFrame,UIListLayout,'Y');
	...
	local ToggleExt = function(bool)
		if bool then
			...
			Lavender:CreateAnimation(ExtElementFrame,0.45,{
				Size = UDim2.new(0, 200, 0, size)
			})

			Lavender:CreateAnimation(DropShadow,0.45,{
				ImageTransparency = 0.750
			})

			Lavender:CreateAnimation(UIStroke,0.45,{
				Transparency = 0
			})
			...
			SPAWN_THREAD = task.spawn(function()
				while true do task.wait(0.1)
					...
					Lavender:CreateAnimation(ExtElementFrame,0.35,{
						Position = ud
					});
				end;
			end)
	...
	UserInputService.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
			if not Lavender:IsMouseOverFrame(ExtElementFrame) and not Lavender.GLOBAL_ENVIRONMENT.IS_HOLD_COLOR_PICKER then
				ToggleExt(false);
			end;
		end;
	end);

	local UIElements: Elements = Lavender:CreateElements(ScrollingFrame,ScrollingFrame.ZIndex,Bindable);

	return UIElements;
end;

function Lavender:CreateColorPicker(ColorBox: Frame,Transparency, Callback)
	Transparency = Transparency or 0;
	Callback = Callback or function() end;

	local OwnWindow = Lavender:GetWindowFromElement(ColorBox);
	...
	local VisibleToggle = function(value)
		if value then
			...
			Lavender:CreateAnimation(ColorPickBox,0.45,{
				ImageTransparency = 0,
				BackgroundTransparency = 0
			});

			Lavender:CreateAnimation(MouseMovement,0.45,{
				ImageTransparency = 0,
			});

			Lavender:CreateAnimation(UIStroke_2,0.45,{
				Transparency = 0
			});

			Lavender:CreateAnimation(ColorRedGreenBlue,0.45,{
				BackgroundTransparency = 0
			});

			Lavender:CreateAnimation(ColorRGBSlide,0.45,{
				BackgroundTransparency = 0
			});

			Lavender:CreateAnimation(UIStroke_3,0.45,{
				Transparency = 0.75
			});

			Lavender:CreateAnimation(ColorOpc,0.45,{
				BackgroundTransparency = 0
			});

			Lavender:CreateAnimation(ColorOptSlide,0.45,{
				BackgroundTransparency = 0
			});

			Lavender:CreateAnimation(UIStroke_4,0.45,{
				Transparency = 0.75
			});

			Lavender:CreateAnimation(UIStroke_5,0.45,{
				Transparency = 0
			});

			Lavender:CreateAnimation(PasteButton,0.45,{
				ImageTransparency = 0.45
			});

			Lavender:CreateAnimation(CopyButton,0.45,{
				ImageTransparency = 0.45
			});

			Lavender:CreateAnimation(hexCode,0.45,{
				BackgroundTransparency = 0.4
			});

			Lavender:CreateAnimation(HexCodeText,0.45,{
	...
	Lavender:AddDragBlacklist(ColorPickerFrame);
	...
	hexCode.BackgroundColor3 = Lavender.Colors.Black
	...
	HexCodeText.FontFace = Lavender.FontSemiBold
	...
	Lavender:CreateHover(CopyButton,function(bool)
		if bool then
			Lavender:CreateAnimation(CopyButton,0.45,{
	...
	Lavender:CreateHover(PasteButton,function(bool)
		if bool then
			Lavender:CreateAnimation(PasteButton,0.45,{
				ImageTransparency = 0.1
			})
	...
	PasteButton.MouseButton1Click:Connect(function()
		if Lavender.GLOBAL_ENVIRONMENT.COLOR_COPY then
			local h,s,v = Lavender.GLOBAL_ENVIRONMENT.COLOR_COPY.RGB:ToHSV();

			Transparency = Lavender.GLOBAL_ENVIRONMENT.COLOR_COPY.OPC;

			OldCode = h;
			CodeH = s;
			CodeV = v;

			HexCodeText.Text = "#" .. tostring(Lavender.GLOBAL_ENVIRONMENT.COLOR_COPY.RGB:ToHex());
			...
			ColorOpc.BackgroundColor3 = Lavender.GLOBAL_ENVIRONMENT.COLOR_COPY.RGB;
	...
	CopyButton.MouseButton1Click:Connect(function()
		Lavender.GLOBAL_ENVIRONMENT.COLOR_COPY = {
	...
	Lavender:NewInput(ColorBox,function()
		VisibleToggle(true);
	end);
	...
	do
		...
		ColorPickerFrame.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				...
				Lavender.GLOBAL_ENVIRONMENT.IS_HOLD_COLOR_PICKER = true;
		...
		ColorPickerFrame.InputEnded:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				IsPressM1 = false;
				Lavender.GLOBAL_ENVIRONMENT.IS_HOLD_COLOR_PICKER = false;
		...
		UserInputService.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				if not Lavender:IsMouseOverFrame(ColorPickerFrame) then
					VisibleToggle(false);
				end;
			end;
		end)
		...
		ColorRedGreenBlue.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				...
				while (UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) or IsPressM1) do task.wait()
					...
					Lavender:CreateAnimation(ColorBox,0.5,{
						BackgroundColor3 = Color
					});

					Lavender:CreateAnimation(ColorOpc,0.35,{
						BackgroundColor3 = Color
					});

					Lavender:CreateAnimation(ColorPickBox,0.5,{
		...
		ColorPickBox.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				...
				while (UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) or IsPressM1) do task.wait();
					...
					Lavender:CreateAnimation(MouseMovement,0.2,nil,{
						Position = UDim2.new(CodeH, 0, 1 - CodeV, 0)
					})
				end
			end
...
function Lavender:RandomStr(len: number): string
	local main = "";

	for i = 1 , len do
		local max = #Lavender.Ascii;
		local rand = math.random(1,max);

		main = main .. Lavender.Ascii:sub(rand,rand);
	end;

	return main;
end;

function Lavender:AddDragBlacklist(Frame: Frame)
	Frame.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.MouseMovement then
			local finder = table.find(Lavender.DragBlacklist , Frame);

			if not finder then
				table.insert(Lavender.DragBlacklist , Frame);
			end;
		end;
	end);
	...
	Frame.InputEnded:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.MouseMovement then
			...
			if finder then
				table.remove(Lavender.DragBlacklist , finder);
			end;
		end;
	end);
end;

function Lavender:ProtectText(Label: TextLabel,Text: string)
	Label.RichText = true;

	local MainText = string.gsub(Text,'.',function(t)
		if not string.find(t,"[<>()\"']") then
			return string.format('<font %s="%s">%s</font>',Lavender:RandomStr(5),Lavender:RandomStr(5),t)
		end;
		return t;
	end);
...
function Lavender:CreateDropdown(Parent: Frame, Default: string | {[string]: boolean}, Multiplier: boolean, AutoUpdate: boolean,Callback: (data: any) -> any)
	local Window = Lavender:GetWindowFromElement(Parent);
	local Data = {};
	...
	Lavender:AddDragBlacklist(DropdownItemFrame);
		...
		if value then
			...
			SPAWN_THREAD = task.spawn(function()
				while true do task.wait()
					...
					Lavender:CreateAnimation(DropdownItemFrame,0.35,{
						Size = UDim2.new(0, 175, 0, math.clamp(baseSize,0,200))
					})
				end;
			end)

			Lavender:CreateAnimation(UIStroke,0.35,{
				Transparency = 0
			})

			Lavender:CreateAnimation(DropShadow,0.35,{
				ImageTransparency = 0.75
			})
		else
			if SPAWN_THREAD then
	...
	local new_button = function()
		...
		db_selected.FontFace = Lavender.FontSemiBold
		db_selected.TextColor3 = Lavender.Colors.Main
	...
	local func;
	local res = Lavender:CreateResponse({
		set_data = function(v)
			Data = v;
		end,
			...
			for i,v in next , Data do
				...
				if Multiplier then
					if (typeof(Default) == 'table' and (Default[v] or table.find(Default,v))) or Default == v then
						Selected[v] = true
						bth.TextColor3 = Lavender.Colors.Main;
					else
	...
	Lavender:NewInput(Parent,function()
		if AutoUpdate then
			res:refresh();
		end;
	...
	UserInputService.InputBegan:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
			if not Lavender:IsMouseOverFrame(DropdownItemFrame) then
				func(false);
				Toggle(false);
			end;
		end;
...
function Lavender:CreateElements(Parent : Frame , ZIndex : number , Event : BindableEvent,SearchAPI: {Path: string,Memory : (name:string) -> any}) : Elements
	local elements = {};
	local FatalWindow = Lavender:GetWindowFromElement(Parent);
	...
	function elements:AddToggle(Config: Toggle)
		...
		Lavender:AddDragBlacklist(Toggle);
		...
		Toggle_Name.FontFace = Lavender.FontSemiBold;
		...
		Lavender:ProtectText(Toggle_Name,Config.Name);

		ValueFrame.Name = "ValueFrame"
		ValueFrame.Parent = Toggle
		ValueFrame.AnchorPoint = Vector2.new(1, 0.5)
		ValueFrame.BackgroundColor3 = Lavender.Colors.Black
		...
		local toggleImg = function(value)
			if value then
				Lavender:CreateAnimation(ValueIcon,0.45,{
					ImageTransparency = 0,
					ImageColor3 = Lavender.Colors.Main,
		...
		local OpcToggle = function(value)
			if value then
				...
				Lavender:CreateAnimation(OptionButton,0.45,{
					ImageTransparency = (Config.Option and 0.600) or 1
				})

				toggleImg(Config.Default)

				Lavender:CreateAnimation(ValueFrame,0.45,{
					BackgroundTransparency = 0,
				})

				Lavender:CreateAnimation(Toggle_Name,0.45,{
		...
		Lavender:CreateHover(ValueFrame,function(b)
		...
		Lavender:NewInput(ValueFrame,function()
			Config.Default = not Config.Default;
			toggleImg(Config.Default);
			Config.Callback(Config.Default)
		end);

		local Respons = Lavender:CreateResponse({
			Rename = function(new_name)
				Toggle_Name.Text = new_name;
				Lavender:ProtectText(Toggle_Name,new_name);
			end,
			GetValue = function()
			...
			Option = (Config.Option and Lavender:CreateOption(OptionButton)) or nil;
		});

		if Config.Flag then
			Lavender.WindowFlags[FatalWindow][Config.Flag.."Toggle"] = Respons;
		end;
	...
	function elements:AddSlider(Config: Slider)
		...
		Lavender:AddDragBlacklist(Slider);
		...
		Slider_Name.FontFace = Lavender.FontSemiBold
		...
		boxli.BackgroundColor3 = Lavender.Colors.Main
		...
		ValueText.FontFace = Lavender.FontSemiBold
		...
		local OpcToggle = function(value)
			if value then
				Lavender:CreateAnimation(boxli,0.45,{
					BackgroundTransparency = 0,
				})
				...
				Lavender:CreateAnimation(Slider_Name,0.45,{
					TextTransparency = 0.200
				})

				Lavender:CreateAnimation(ValueText,0.45,{
		...
		local function update(Input)
			...
			local Value = Lavender:Rounding(Main,Config.Round);
			...
			ValueFrame.InputEnded:Connect(function(Input)
				if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
					if UserInputService.TouchEnabled then
						if not Lavender:IsMouseOverFrame(ValueFrame) then
							IsHold = false
						end;
					else
						IsHold = false
		...
		if Config.Flag then
			Lavender.WindowFlags[FatalWindow][Config.Flag.."Slider"] = Respons;
		end;
	...
	function elements:AddButton(Config: Button)
		...
		Button.BackgroundColor3 = Lavender.Colors.Black
		Button.BorderColor3 = Color3.fromRGB(0, 0, 0)
		Button.BorderSizePixel = 0
		Button.Size = UDim2.new(1, -25, 0, 25)
		Button.ZIndex = ZIndex + 1
		Lavender:AddDragBlacklist(Button);
		...
		Button_Name.FontFace = Lavender.FontSemiBold
		...
		Lavender:ProtectText(Button_Name,Config.Name);
		...
		local OpcToggle = function(value)
			if value then
				Lavender:CreateAnimation(Button_Name,0.45,{
					TextTransparency = 0.4,
				})

				Lavender:CreateAnimation(Button,0.45,{
					BackgroundTransparency = 0,
				})
			else
				Lavender:CreateAnimation(Button_Name,0.45,{
		...
		Lavender:CreateHover(Button,function(value)
			if value then
				Lavender:CreateAnimation(Button,0.45,{
				...
				Lavender:CreateAnimation(Button,0.45,{
					BackgroundColor3 = Lavender.Colors.Black
				})
			end;
		end)

		Lavender:NewInput(Button,function()
			Config.Callback();
		end)

		return Lavender:CreateResponse({
			Rename = function(new_name)
				Button_Name.Text = new_name
				Lavender:ProtectText(Button_Name,new_name);
			end,
			GetValue = function()
				return Config.Default;
			end,
	...
	function elements:AddColorPicker(Config: ColorPicker)
		...
		Lavender:AddDragBlacklist(ColorPicker);
		...
		ColorPicker_Name.FontFace = Lavender.FontSemiBold
		...
		local res = Lavender:CreateColorPicker(ValueFrame,Config.Transparency,function(rgb,opc)
		...
		local OpcToggle = function(value)
			if value then
				Lavender:CreateAnimation(ColorPicker_Name,0.45,{
		...
		if Config.Flag then
			Lavender.WindowFlags[FatalWindow][Config.Flag.."ColorPicker"] = Respons;
		end;
	...
	function elements:AddDropdown(Config: Dropdown)
		...
		Lavender:AddDragBlacklist(Dropdown);
		...
		Dropdown_Name.FontFace = Lavender.FontSemiBold
		...
		Lavender:ProtectText(Dropdown_Name,Config.Name);
		...
		Value_Text.FontFace = Lavender.FontSemiBold
		...
		local res;
		res = Lavender:CreateDropdown(ValueFrame,Config.Default,Config.Multi,Config.AutoUpdate,function(args)
			Config.Default = args;
			Value_Text.Text = DataParser(Config.Default);
		...
		res:on_toggle(function(b)
			if b then
				Lavender:CreateAnimation(icon,0.35,{
					Rotation = -180,
					ImageColor3 = Lavender.Colors.Main
				})
			else
		...
		local OpcToggle = function(value)
			if value then
				Lavender:CreateAnimation(Dropdown_Name,0.45,{
					TextTransparency = 0.2,
				})
				...
				Lavender:CreateAnimation(icon,0.45,{
					ImageTransparency = 0
				})

				Lavender:CreateAnimation(Value_Text,0.45,{
		...
		local Respons = Lavender:CreateResponse({
			Rename = function(new_name)
				...
				Lavender:ProtectText(Dropdown_Name,new_name);
			end,
			GetValue = function()
				return Config.Default;
			end,
		...
		if Config.Flag then
			Lavender.WindowFlags[FatalWindow][Config.Flag.."Dropdown"] = Respons;
		end;
	...
	function elements:AddKeybind(Config: Keybind)
		...
		Lavender:AddDragBlacklist(Keybind);
		...
		Keybind_Name.FontFace = Lavender.FontSemiBold
		...
		Lavender:ProtectText(Keybind_Name,Config.Name);
		...
		local OpcToggle = function(value)
			if value then
				Lavender:CreateAnimation(Keybind_Name,0.45,{
					TextTransparency = 0.2,
				})
		...
		local Respons = Lavender:CreateResponse({
			Rename = function(new_name)
				...
				Lavender:ProtectText(Keybind_Name,new_name);
			end,
			GetValue = function()
				return Config.Default;
			end,
		...
		if Config.Flag then
			...
			Lavender.WindowFlags[FatalWindow][Config.Flag.."Keybind"] = Respons;
		end;
...
function Lavender:CreateConfigWindow(Root: ScreenGui , Fatal , Button: ImageButton)
	...
	Lavender:AddDragBlacklist(ConfigWindowFrame);
	...
	local ElementToggle = function(value)
		...
		if value then
			...
			Lavender:CreateAnimation(ConfigWindowFrame,0.3,{
				Size = UDim2.new(0, 185, 0, 195),
			})

			Lavender:CreateAnimation(UIStroke,0.3,{
				Transparency = 0
			})

			Lavender:CreateAnimation(DropShadow,0.3,{
				ImageTransparency = 0.750
			})

			Lavender:CreateAnimation(ScrollingFrame,0.3,{
				BackgroundTransparency = 0
			})

			Lavender:CreateAnimation(UIStroke_2,0.3,{
				Transparency = 0.750
			})

			Lavender:CreateAnimation(UIStroke_3,0.3,{
				Transparency = 0.750
			})

			Lavender:CreateAnimation(SpaceBox,0.3,{
				BackgroundTransparency = 0
			})

			Lavender:CreateAnimation(InputFrame,0.3,{
				BackgroundTransparency = 0
			})

			Lavender:CreateAnimation(TextBox,0.3,{
				TextTransparency = 0.5
			})

			Lavender:CreateAnimation(AddConfigButton,0.3,{
				ImageTransparency = 0.500
			})

			Lavender:CreateAnimation(GetConfigButton,0.3,{
				ImageTransparency = 0.500
			})

			Lavender:CreateAnimation(SaveConfigButton,0.3,{
				ImageTransparency = 0.500
			})

			Lavender:CreateAnimation(DeleteButton,0.3,{
			...
			Lavender:CreateAnimation(ConfigWindowFrame,0.275,{
	...
	ScrollingFrame.Active = true
	ScrollingFrame.BackgroundColor3 = Lavender.Colors.Black
	ScrollingFrame.BorderColor3 = Color3.fromRGB(0, 0, 0)
	ScrollingFrame.BorderSizePixel = 0
	...
	InputFrame.BackgroundColor3 = Lavender.Colors.Black
	...
	TextBox.ZIndex = 209
	TextBox.FontFace = Lavender.FontSemiBold
	TextBox.Text = ""
	...
	DeleteButton.ImageColor3 = Lavender.Colors.Main
	...
	Lavender:CreateHover(DeleteButton,function(bool)
		if bool then
			Lavender:CreateAnimation(DeleteButton,0.3,{
				ImageTransparency = 0;
			})
	...
	Lavender:CreateHover(SaveConfigButton,function(bool)
		if bool then
			Lavender:CreateAnimation(SaveConfigButton,0.3,{
	...
	Lavender:CreateHover(GetConfigButton,function(bool)
		if bool then
			Lavender:CreateAnimation(GetConfigButton,0.3,{
				ImageTransparency = 0.15;
			})
	...
	Lavender:CreateHover(AddConfigButton,function(bool)
		if bool then
			Lavender:CreateAnimation(AddConfigButton,0.3,{
				ImageTransparency = 0.15;
			})
	...
	game:GetService('UserInputService').InputBegan:Connect(function(Input,Typing)
		if Input.UserInputType == Enum.UserInputType.Touch or Input.UserInputType == Enum.UserInputType.MouseButton1 then
			if UIToggle and not Lavender:IsMouseOverFrame(ConfigWindowFrame) then
				ElementToggle(false);
			end;
		end;
	end);
	...
	local new_button = function()
		...
		db_selected.Name = Lavender:RandomString()
		db_selected.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
		db_selected.BackgroundTransparency = 1.000
		db_selected.BorderColor3 = Color3.fromRGB(0, 0, 0)
		db_selected.BorderSizePixel = 0
	...
	res = Lavender:CreateResponse({
		...
		Refresh = function()
			...
			for i,v in next, Configs do
				...
				if tostring(import.Name) == tostring(v) or import.Name == v then
					...
					import.Name = v;
					import.Element.TextColor3 = Lavender.Colors.Main;
					button.Text = "  "..tostring(v);
				end;
				...
				button.MouseButton1Click:Connect(function()
					...
					import.Element = button;
					import.Name = v;
					button.TextColor3 = Lavender.Colors.Main;
					button.Text = "  "..tostring(v);
				end);
			end;
		end,
		...
		Init = function(Name: string,Folder: string)
			if not isfolder(Folder or "Lavender") then
				makefolder(Folder or "Lavender");	
			end;

			if not isfolder((Folder or "Lavender").."/Config") then
				makefolder((Folder or "Lavender").."/Config");	
			end;

			local cfgPath = (Folder or "Lavender").."/Config/"..tostring(Name);
...
function Lavender.new(Window: Window)
	Window = Window or {};
	Window.Name = Window.Name or "Lavender";
	...
	Fatal.Notifier = Lavender.__NOTIFIER_CACHE or Lavender:CreateNotifier();

	local Lavenderwin = Instance.new("ScreenGui")
	...
	Lavender.WindowFlags[Lavenderwin] = {};

	Lavender:ScrollSignal(tbc,UIListLayout,'X');
	...
	FatalFrame:GetPropertyChangedSignal("BackgroundTransparency"):Connect(function()
		if FatalFrame.BackgroundTransparency >= 0.99 then
			Lavenderwin.Enabled = false;
		else
			Lavenderwin.Enabled = true;
		end;
	end);
	...
	local ToggleUI = function(bool)
		...
		if bool then
			...
			Lavender:CreateAnimation(FatalFrame,0.15,{
				Size = Window.Scale,
				BackgroundTransparency = 0,
			})

			Lavender:CreateAnimation(Header,0.5,{
				BackgroundTransparency = 0,
			})

			Lavender:CreateAnimation(HeaderLine,0.5,{
				BackgroundTransparency = 0,
			})

			Lavender:CreateAnimation(Bottom,0.5,{
				BackgroundTransparency = 0,
			})

			Lavender:CreateAnimation(HeaderLine_2,0.5,{
				BackgroundTransparency = 0,
			})

			Lavender:CreateAnimation(HeaderLineShadow,0.5,{
				BackgroundTransparency = 0.5,
			})

			Lavender:CreateAnimation(HeaderLineShadow_2,0.5,{
				BackgroundTransparency = 0.5,
			})

			Lavender:CreateAnimation(InfoButton,0.25,{
				ImageTransparency = 0.5
			})

			Lavender:CreateAnimation(SearchButton,0.25,{
				ImageTransparency = 0.5
			})

			Lavender:CreateAnimation(SaveButton,0.25,{
				ImageTransparency = 0.5
			})

			Lavender:CreateAnimation(HeaderText,0.35,{
				TextStrokeTransparency = 0.640,
				TextTransparency = 0
			})

			Lavender:CreateAnimation(UserIcon,0.45,{
				ImageTransparency = 0,
				Position = UDim2.new(1, -10,0.5, 0)
			})

			Lavender:CreateAnimation(User_name,0.35,{
				TextTransparency = 0,
				Position = UDim2.new(1, -40,0, 3)
			})

			Lavender:CreateAnimation(expire_days,0.5,{
				TextTransparency = 0,
			...
			Lavender:CreateAnimation(InfoButton,0.1,Enum.EasingStyle.Back,{
				Position = UDim2.new(1, -5,0.5, 0)
			})

			Lavender:CreateAnimation(SearchButton,0.1,Enum.EasingStyle.Back,{
				Position = UDim2.new(0,10,0.5, 0)
			})

			Lavender:CreateAnimation(SaveButton,0.1,Enum.EasingStyle.Back,{
			...
			table.clear(Lavender.DragBlacklist);

			for i,v in next , Fatal.Menus do
				v.ValueSelect(false);
			end;

			Lavender:CreateAnimation(SaveButton,0.35,{
				Position = UDim2.new(0,35,1, 20)
			})

			Lavender:CreateAnimation(SearchButton,0.35,{
				Position = UDim2.new(0,10,1, 20)
			})

			Lavender:CreateAnimation(InfoButton,0.35,{
			...
			Lavender:CreateAnimation(UserIcon,0.35,{
				ImageTransparency = 1,
				Position = UDim2.new(1, -10,0.5, 0)
			})
			...
			Lavender:CreateAnimation(expire_days,0.1,{
				TextTransparency = 1,
				Position = UDim2.new(1, -40,0, 16)
			})
			...
			Lavender:CreateAnimation(FatalFrame,0.75,{
				BackgroundTransparency = 1,
			})
	...
	local existingGui = game:GetService("CoreGui"):FindFirstChild("Lavender")
if existingGui then
    existingGui:Destroy()
		end

	Lavenderwin.Name = "Lavender"
	Lavenderwin.Parent = game.CoreGui
	Lavenderwin.ResetOnSpawn = false;
	Lavenderwin.IgnoreGuiInset = true;
	Lavenderwin.ZIndexBehavior = Enum.ZIndexBehavior.Global;

	


	table.insert(Lavender.Windows,Lavenderwin)

	protect_gui(Lavenderwin);

	FatalFrame.Active = true;
	FatalFrame.Name = "FatalFtame"
	FatalFrame.Parent = Lavenderwin
...
Open_7274.Parent = Lavenderwin
	...
	Lavender:Drag(FatalFrame,FatalFrame,0.1);
	...
	function Fatal:GetFlags()
		return Lavender.WindowFlags[Lavenderwin];
	end;
	...
	function Fatal:AddConfig()
		...
		Lavender:CreateHover(ConfigButton,function(bool)
			if bool then
				Lavender:CreateAnimation(ConfigButton,0.3,{
					ImageTransparency = 0.2500;
				})
			else
				Lavender:CreateAnimation(ConfigButton,0.3,{
		...
		return Lavender:CreateConfigWindow(Lavenderwin,Fatal,ConfigButton);
	end;
	...
	function Fatal:LoadConfig(config)
		...
		for i,v in next , config do
			if i ~= "Info" then
				local Element = Lavender.WindowFlags[Lavenderwin][i];
	...
	function Fatal:AddMenu(Menu : Menu)
		...
		Icon.SizeConstraint = Enum.SizeConstraint.RelativeYY
		Icon.ZIndex = 5
		Icon.Image = Lavender:GetIcon(Menu.Icon);
		Icon.ImageColor3 = Lavender.Colors.Main
		...
		local text_size = Lavender:GetTextSize(menu_name);
		...
		BindEvent.Name = Lavender:RandomString();

		if not Menu.AutoFill then
			Lavender:ScrollSignal(Right,UIListLayout_3,'Y');
			Lavender:ScrollSignal(Center,UIListLayout_2,'Y');
			Lavender:ScrollSignal(Left,UIListLayout,'Y');
		else
		...
		Fatal.Signal.Event:Connect(function(Bool)
			if Bool then
				Lavender:CreateAnimation(MenuButton,0.5,{
					BackgroundTransparency = (MenuLiber.Visible and 0) or 1
				})

				Lavender:CreateAnimation(UIStroke,0.5,{
					Transparency = 0.950
				})

				Lavender:CreateAnimation(Left,0.3,{
					Position = UDim2.new(0.175, 0, 0.5, 0)
				})

				Lavender:CreateAnimation(Center,0.4,{
					Position = UDim2.new(0.5, 0, 0.5, 0)
				})

				Lavender:CreateAnimation(Right,0.5,{
					Position = UDim2.new(0.825, 0, 0.5, 0)
				})

				Lavender:CreateAnimation(Icon,0.5,{
					ImageTransparency = (MenuLiber.Visible and 0.15) or 0.5
				})

				Lavender:CreateAnimation(menu_name,0.5,{
					TextStrokeTransparency = 0.900,
					TextTransparency = (MenuLiber.Visible and 0.15) or 0.5
				})
			else
				Lavender:CreateAnimation(Left,0.5,{
					Position = UDim2.new(0.175, 0, 0.5, 1)
				})

				Lavender:CreateAnimation(Center,0.5,{
		...
		Lavender:CreateHover(MenuButton,function(bool)
		...
		Lavender:NewInput(MenuButton,function()
			for i,v in next , Fatal.Menus do
		...
		function MenuLib:AddPreview(Config: Preview)
			...
			PreviewName.FontFace = Lavender.FontSemiBold
			...
			local Toggle = function(v)
				if v then
					Lavender:CreateAnimation(Preview,0.5,{
						BackgroundTransparency = 0
					})

					Lavender:CreateAnimation(PreviewName,0.5,{
						TextStrokeTransparency = 0.750,
						TextTransparency = 0
					})
					
					Lavender:CreateAnimation(Main,0.5,{
						BackgroundTransparency = 0
					})
					...
					Lavender:CreateAnimation(MainBlock,0.5,{
		...
		function MenuLib:AddListBox(Config : Listbox)
			...
			Lavender:AddDragBlacklist(ScrollingFrame);
			...
			SearchBox.BackgroundColor3 = Lavender.Colors.Black
			...
			TextBox.BackgroundColor3 = Lavender.Colors.Black
			TextBox.BackgroundTransparency = 1.000
			TextBox.BorderColor3 = Color3.fromRGB(0, 0, 0)
			TextBox.BorderSizePixel = 0
			...
			ListName.ZIndex = 10
			ListName.FontFace = Lavender.FontSemiBold
			...
			local Toggle = function(v)
				if v then
					Lavender:CreateAnimation(ListBox,0.5,{
						BackgroundTransparency = 0
					})
					...
					Lavender:CreateAnimation(UIStroke_2,0.5,{
						Transparency = 0.75
					})

					Lavender:CreateAnimation(UIStroke_3,0.5,{
						Transparency = 0.75
					})

					Lavender:CreateAnimation(SearchBox,0.5,{
						BackgroundTransparency = 0
					})

					Lavender:CreateAnimation(TextBox,0.5,{
						TextTransparency = 0
					})
					...
					Lavender:CreateAnimation(ScrollingFrame,0.45,{
						BackgroundTransparency = 0,
					...
					table.foreach(ScrollingFrame:GetChildren(),function(i,v)
						if v:IsA('TextButton') then
							Lavender:CreateAnimation(v,0.45,{
								TextTransparency = 0.5
							});
						end;
					end);

					Lavender:CreateAnimation(ListName,0.5,{
				...
				Refresh = function()
					...
					for i,v in next , Config.Values do
						...
						if Config.Multi then
							...
							Lavender:CreateHover(bth,function(bool)
								if not Config.Default[v] then
									if bool then
										Lavender:CreateAnimation(bth,0.25,{
											TextTransparency = 0;
										})
									else
										Lavender:CreateAnimation(bth,0.25,{
			...
			if Config.Flag then
				Lavender.WindowFlags[Lavenderwin][Config.Flag.."ListBox"] = res;
			end;
		...
		function MenuLib:AddSection(Config : Section)
			...
			local Toggle = function(v)
				if v then
					Lavender:CreateAnimation(Section,0.5,{
						BackgroundTransparency = 0
					})
					...
					Lavender:CreateAnimation(SectionName,0.5,{
						TextStrokeTransparency = 0.750,
						TextTransparency = 0
					})
				else
			...
			SectionName.FontFace = Lavender.FontSemiBold;
			...
			UIListLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
				...
				if not Menu.AutoFill then
					Lavender:CreateAnimation(Section,0.25,{
			...
			return Lavender:CreateElements(Elements,Elements.ZIndex,BindEvent,{
				Path = Menu.Name .. " > ".. Config.Name,
	...
	do
		...
		Lavender:CreateHover(SaveButton,function(bool)
			if bool then
				Lavender:CreateAnimation(SaveButton,0.5,{
					ImageTransparency = 0.1
				})
			else
				Lavender:CreateAnimation(SaveButton,0.5,{
		...
		Lavender:CreateHover(InfoButton,function(bool)
			if bool then
				Lavender:CreateAnimation(InfoButton,0.5,{
					ImageTransparency = 0.1
				})
			else
				Lavender:CreateAnimation(InfoButton,0.5,{
	...
	do
		...
		Lavender:AddDragBlacklist(SearchFrame);
		...
		local SearchToggle = function(value)
			if value then
				...
				Lavender:CreateAnimation(SearchFrame,0.35,{
					Size = searchScale
				})
				...
				Lavender:CreateAnimation(SearchBox,0.35,{
					BackgroundTransparency = 0
				})
		...
		SearchFrame.Parent = Lavenderwin;
		...
		Lavender:CreateHover(SearchButton,function(bool)
			if bool then
				Lavender:CreateAnimation(SearchButton,0.5,{
					ImageTransparency = 0.1
				})
			else
				Lavender:CreateAnimation(SearchButton,0.5,{
		...
		local get_button = function(Name,Path,TAB_WARP)
			...
			ResultFrame.BackgroundColor3 = Lavender.Colors.Black
			...
			FeatureName.FontFace = Lavender.FontSemiBold
			...
			FeaturePath.FontFace = Lavender.FontSemiBold
			...
			local button = Lavender:NewInput(ResultFrame);

			Lavender:CreateHover(button,function(bool)
				if bool then
					Lavender:CreateAnimation(ResultFrame,0.5,{
						BackgroundTransparency = 0
					})
				else
					Lavender:CreateAnimation(ResultFrame,0.5,{
		...
		SearchButton.MouseButton1Click:Connect(function()
			...
			table.foreach(Fatal.ElementContents,function(i,v)
				...
				SearchInformation[v.Path.." - "..Lavender:RandomString()] = {
					root = button,
					callback = function()
		...
		UserInputService.InputBegan:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
				if not Lavender:IsMouseOverFrame(SearchFrame) then
					SearchToggle(false);
				end
			end
		end)
	...
	function Fatal:GetButton()
		...
		backpack.MouseEnter:Connect(function()
			Lavender:CreateAnimation(backpack,0.3,nil,{
				BackgroundColor3 = Lavender.Colors.Main,
...
function Lavender:Loader(Config: Loader)
	Config = Config or {};
	Config.Name = Config.Name or "Lavender";
	...
	Lavender:CreateAnimation(Blur,1,{
		Size = 60
	})

	Lavender:CreateAnimation(BlackFrame,0.5,{
	...
	local createText = function(TEXT)
		...
		local textsize = Lavender:GetTextSize(ASCII);
	...
	do
		...
		local textsize = Lavender:GetTextSize(StartText);
		...
		Lavender:CreateAnimation(StartText,0.45,{
			TextTransparency = 0
		})

		Lavender:CreateAnimation(UIScale,0.5,{
			Scale = Config.Scale;
		});

		task.wait(0.45);

		Lavender:CreateAnimation(StartText,0.35,{
		...
		for i,v in next , PosText do
			if i > 1 then
				Lavender:CreateAnimation(v.Text,0.65,{
					Position = UDim2.new(0.5,0,0.5,0);
					TextTransparency = 0
				})
			end;
		...
		Lavender:CreateAnimation(StartText,1.5,{
			TextTransparency = 1
		})

		for i,v in next , PosText do
			Lavender:CreateAnimation(v.Text,1.5,{
				TextTransparency = 1
			})
		end;

		Lavender:CreateAnimation(Blur,1.5,{
			Size = 0
		})

		Lavender:CreateAnimation(BlackFrame,1.5,{
			BackgroundTransparency = 1
		})
...
function Lavender:CreateNotifier(): Notifier
	if Lavender.__NOTIFIER_CACHE then return Lavender.__NOTIFIER_CACHE; end;
	...
	local res = Lavender:CreateResponse({
		Notify = function(Config: Notify)
			...
			Icon.BackgroundColor3 = Lavender.Colors.Main
			Icon.BackgroundTransparency = 1.000
			Icon.BorderColor3 = Color3.fromRGB(0, 0, 0)
			Icon.BorderSizePixel = 0
			Icon.Position = UDim2.new(0, 7, 0, 7)
			Icon.Size = UDim2.new(0, 18, 0, 18)
			Icon.Image = Lavender:GetIcon(Config.Icon);
			Icon.ImageColor3 = Lavender.Colors.Main
			...
			HeaderText.FontFace = Lavender.FontSemiBold
			HeaderText.Text = Config.Title
			HeaderText.TextColor3 = Lavender.Colors.Main
			...
			BodyText.ZIndex = 55
			BodyText.FontFace = Lavender.FontSemiBold
			...
			local updateScale = function()
				local TitleScale = Lavender:GetTextSize(HeaderText,Enum.Font.GothamBold);
				local ContentScale = Lavender:GetTextSize(BodyText,Enum.Font.GothamMedium);
				...
				Lavender:CreateAnimation(notify_block,0.35,{
					Position = UDim2.new(0.5, 0, 0, 0)
				});
			end;

			task.delay(0.1,function()
				Lavender:CreateAnimation(notify_block,0.25,{
					BackgroundTransparency = 0.1
				})

				Lavender:CreateAnimation(UIStroke,0.25,{
					Thickness = 2.500,
					Transparency = 0.900
				})

				Lavender:CreateAnimation(Icon,0.25,{
					ImageTransparency = 0
				})

				Lavender:CreateAnimation(HeaderText,0.25,{
					TextTransparency = 0.200
				})

				Lavender:CreateAnimation(BodyText,0.25,{
			...
			task.delay(Config.Duration + 0.25,function()
				...
				task.delay(0.35,function()
					Lavender:CreateAnimation(notify,0.5,{
						Size = UDim2.new(0,0,0,0)
					})
	...
	Lavender.__NOTIFIER_CACHE = res;

	return res;
end;

Lavender.Lavender_PID = Lavender:RandomString();
